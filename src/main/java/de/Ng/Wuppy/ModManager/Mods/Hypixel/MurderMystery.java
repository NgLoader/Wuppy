package de.Ng.Wuppy.ModManager.Mods.Hypixel;
...
import de.Ng.Wuppy.ModManager.Mods.Hypixel.MurderMystery.ClassType;
import de.Ng.Wuppy.ModManager.Mods.Hypixel.MurderMystery.CustomPlayer;
import de.Ng.Wuppy.ModManager.Modul.Trigger;
import de.Ng.Wuppy.ModManager.Modul.Triggers.TriggerScoreboard;
import de.Ng.Wuppy.ModManager.addons.AddonPictureInPicture;
...
public class MurderMystery extends Mod {
	...
	public MurderMystery(ModManager modManager) {
		super(modManager, "murdermystery.name", "murdermystery.description", ModCategory.HYPIXEL, DectectType.UNDETECTABLE);
	}
	...
	public void onInit() {
		...
		murderItems = new Item[] { Items.IRON_SWORD, Items.STONE_SWORD, Items.IRON_SHOVEL, Items.STICK, Items.STONE_SHOVEL, Items.BLAZE_ROD, Items.CARROT_ON_A_STICK, Items.BONE, Items.GOLDEN_SWORD, Items.DIAMOND_SWORD, Items.DIAMOND_HOE };
	...
	public void reset() {
		...
		AddonPictureInPicture.resetPlayers();
	...
	public void onChat(ClientChatReceivedEvent event) { //BEBUG
		if(mc.isSingleplayer())
			if(event.getMessage().getFormattedText().contains("add"))
				mc.world.getScoreboard().addScoreObjective("MurderMystery", IScoreCriteria.AIR);
			...
			mc.world.loadedEntityList.forEach(entity -> {
				if(entity != null) {
					if(entity instanceof EntityItem) {
						...
						if(entityArmorStand.isInvisible() && (entityArmorStand.getHeldItemOffhand().getItem() == Items.BOW || entityArmorStand.getHeldItemMainhand().getItem() == Items.BOW) && !detectivBows.contains(entity.getCachedUniqueIdString()))
						...
						} else if(hasItemInHand(entityPlayer, Items.BOW)) {
	...
	private void addGoldDrop(Scoreboard scoreboard, EntityItem entityItem) {
		...
		if(!goldDrops.contains(entityItem.getCachedUniqueIdString())) {
	...
	class CustomPlayer {
		...
		public void updatePlayer(EntityPlayer entityPlayer, Scoreboard scoreboard) {
			...
			case MURDER:
				...
				AddonPictureInPicture.addPlayer(name);
				...
				break;
			case DETECTIV:
				AddonPictureInPicture.removePlayer(name);
